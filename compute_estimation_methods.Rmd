...
We will do the same as above, but we will also estimate nu:
...
We will now also look at `nu=1.4` and `m=6`:


Let us look, for example, at `nu = 1.4` and `m=6`. We start by looking at the timings. First for our rational approximation:

```{r}
res_lik_est_nu[["rat"]][["1.4"]]$timings[[6]]
```

Now for the nearest neighbor Gaussian model:

```{r}
res_lik_est_nu[["nngp"]][["1.4"]]$timings[[6]]
```

Let us proceed as above and summarize the results as a `data.frame`:

```{r}
res_est_nu <- data.frame(error = c(sqrt((loc[2]-loc[1])*sum((res_lik_est_nu[["true"]][["1.4"]] -
                  res_lik_est_nu[["nngp"]][["1.4"]]$post_mean[[6]])^2)), 
                  sqrt((loc[2]-loc[1])*sum((res_lik_est_nu[["true"]][["1.4"]] - 
                  res_lik_est_nu[["rat"]][["1.4"]]$post_mean[[6]])^2))),
                  time_optimization = c(res_lik_est_nu[["nngp"]][["1.4"]]$timings[[6]][["Optimization"]],
                  res_lik_est_nu[["rat"]][["1.4"]]$timings[[6]][["Optimization"]]),
                  time_posterior_mean = c(res_lik_est_nu[["nngp"]][["1.4"]]$timings[[6]][["Posterior_mean"]],
                  res_lik_est_nu[["rat"]][["1.4"]]$timings[[6]][["Posterior_mean"]]),
                  time_compute_precision = c(res_lik_est_nu[["nngp"]][["1.4"]]$timings[[6]][["Precision_computation"]],
                  res_lik_est_nu[["rat"]][["1.4"]]$timings[[6]][["Precision_computation"]]),
                  count = c(res_lik_est_nu[["nngp"]][["1.4"]]$timings[["6"]][["Optimization_counts"]],
                  res_lik_est_nu[["rat"]][["1.4"]]$timings[["6"]][["Optimization_counts"]]),
                  row.names = c("NN", "Rat"))
res
```
...
```{r}
plot(loc,res_lik_est_nu[["true"]][["1.4"]], type = "l", main = "black true, red nn, green markov")
points(loc[obs_ind],res_lik_est_nu[["Y"]][["1.4"]])
lines(loc,res_lik_est_nu[["nngp"]][["1.4"]]$post_mean[[6]],col="red")
lines(loc,res_lik_est_nu[["rat"]][["1.4"]]$post_mean[[6]],col="green")
```