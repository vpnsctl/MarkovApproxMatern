...
We will now compute the timings using the `compute_likelihood_rat()` and `compute_likelihood_nn()` functions.

```{r}
res <- compute_timings_likelihood(loc, obs_ind, nu_vec, kappa, sigma, sigma_e, m, m_nngp_fun)
```
...
```{r}
res[["rat"]][["1.4"]]$timings[[6]]
```

Now for the nearest neighbor Gaussian model:

```{r}
res[["nngp"]][["1.4"]]$timings[[6]]
```

Let us summarize as a `data.frame`:

```{r}
res <- data.frame(error = c(sqrt((loc[2]-loc[1])*sum((mu-res[["nngp"]][["1.4"]]$post_mean[[6]])^2)), 
              sqrt((loc[2]-loc[1])*sum((mu-res[["rat"]][["1.4"]]$post_mean[[6]])^2))),
                  time_optimization = c(res[["nngp"]][["1.4"]]$timings[[6]][["Optimization"]],
                  res[["rat"]][["1.4"]]$timings[[6]][["Optimization"]]),
                  time_posterior_mean = c(res[["nngp"]][["1.4"]]$timings[[6]][["Posterior_mean"]],
                  res[["rat"]][["1.4"]]$timings[[6]][["Posterior_mean"]]),
                  time_compute_precision = c(res[["nngp"]][["1.4"]]$timings[[6]][["Precision_computation"]],
                  res[["rat"]][["1.4"]]$timings[[6]][["Precision_computation"]]),
                  count = c(res[["nngp"]][["1.4"]]$timings[["6"]][["Optimization_counts"]],
                  res[["rat"]][["1.4"]]$timings[["6"]][["Optimization_counts"]]),
                  row.names = c("NN", "Rat"))
res
```
...
lines(loc,res[["nngp"]][["1.4"]]$post_mean[[6]],col="red")
lines(loc,res[["rat"]][["1.4"]]$post_mean[[6]],col="green")
```