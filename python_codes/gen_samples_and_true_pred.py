...
# Compute the true mean and posterior std. dev.
def compute_true_mu_and_sigma(Sigma_row, obs_ind, sigma_e, Y):
    # Full covariance matrix
    Sigma_full = tf.linalg.LinearOperatorToeplitz(row=Sigma_row, col=Sigma_row).to_dense()
    
    # Submatrix corresponding to observed indices
    Sigma = tf.gather(Sigma_full, obs_ind, axis=1)
    
    # Adjust Sigma_row for measurement noise
    ...
    # Submatrix of Sigma_hat corresponding to observed indices
    ...
    Sigma_hat_inv = tf.linalg.inv(Sigma_hat)
    Sigma_post = Sigma_full - tf.matmul(Sigma, tf.matmul(Sigma_hat_inv, tf.transpose(Sigma)))
    
    # Compute posterior standard deviation (sqrt of diagonal)
    sigma = tf.sqrt(tf.linalg.diag_part(Sigma_post))
...
true_sigma_result = np.zeros((tf.size(nu_vec), n_rep, n))  # Store posterior standard deviation

sim_data_nu = np.zeros((n_rep, n_obs))
true_mean_nu = np.zeros((n_rep, n))
true_sigma_nu = np.zeros((n_rep, n))  # Store posterior standard deviation for current nu
...
output_dir = f'python_codes/partial_results'
...
for idx, nu in enumerate(nu_vec):
    ...
    if os.path.exists(filename):
        with h5py.File(filename, 'r') as f:
            ...
            true_sigma_result[idx, :, :] = f['true_sigma_nu'][:]  # Load sigma if it exists
            if n != n_obs:
        ...
        with h5py.File(filename, 'w') as f:
            ...
            f.create_dataset('true_sigma_nu', data=true_sigma_nu)  # Save sigma results
            if n != n_obs:
...
filename = f'simulation_results_n{n}_nobs{n_obs}_range{range_value}_sigmae{sigma_e:.2f}.h5'

with h5py.File(filename, 'w') as f:
    f.create_dataset('sim_data_result', data=sim_data_result)
    f.create_dataset('true_mean_result', data=true_mean_result)
    f.create_dataset('true_sigma_result', data=true_sigma_result)  # Save sigma results
