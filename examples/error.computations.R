...
error.computations_test <- function(range, sigma, sigma.e, n, n.obs, samples.fourier, loc, nu.vec, m.vec, Dists) {

    m.vec <- 1:6
    err.rat <- matrix(0,nrow= length(nu.vec), ncol = length(m.vec))
    range <- range * max(loc)
