...
partial_results_folder   <- "kl_tables/partial_results"

if (!dir.exists("calibration_results")) {
  dir.create("calibration_results")
}
if (!dir.exists("kl_tables")) {
  dir.create("kl_tables")
}
if (!dir.exists(partial_results_folder)) {
  dir.create(partial_results_folder)
}

compute_kl_divergence <- function(Sigma.t, mu_t, Sigma_approx, mu_approx) {
        ...
        kl.approx <- NA
            ...
            tryCatch({
                ...
                Sigma_approx_inv <- solve(Sigma_approx)
                log_det_approx <- determinant(Sigma_approx, logarithm=TRUE)$modulus

                temp <- Sigma_approx_inv %*% Sigma.t
                trace_term <- sum(diag(temp))
                dim <- nrow(Sigma.t)
                diff_mean_term <- as.double((mu_approx - mu_t) %*% Sigma_approx_inv %*% (mu_approx - mu_t))
                kl.approx <- as.double(0.5 * (trace_term - dim  + diff_mean_term + log_det_approx - log_det_true))              
            }, error=function(e) {
                warning(paste("Error computing KL divergence"))
                kl.approx <- NA
            })
    print("KL divergence computed")
    print(kl.approx)
    return(kl.approx)
}
...
sigma        <- 1
sigma.e      <- 0.1 # sqrt(0.1) # or 0.1
n            <- c(0:10, seq(20,50,by=10), seq(75,250,by=25), 300, 400, 500,
...
for (j in seq_len(n.rep)) {
  ...
  for (i in seq_along(n)) {
    ...
    partial_file <- file.path(
      ...
      paste0("kl_partial_result_rep", j,
             "_n", n[i],
             "_range", range,
             "_sigmaE", sigma.e,
             "_nu", nu, ".RDS")
    ...
    for(k in seq_along(m_vec)) {
      ...
      rat_kl <- compute_kl_divergence(post_cov_true, post_mean_true, Sigma_rat, mu_rat)
      
      err_rat[k, i] <- err_rat[k, i] + rat_kl
      ...
      nngp_kl <- compute_kl_divergence(post_cov_true, post_mean_true, Sigma_nngp, mu_nngp)

      err_nngp[k, i] <- err_nngp[k, i] + nngp_kl
      ...
      mu_fem    <- as.vector(post_fem_mat$mu.fem)
      Sigma_fem <- as.matrix(post_fem_mat$Sigma.fem)

      fem_kl <- compute_kl_divergence(post_cov_true, post_mean_true, Sigma_fem, mu_fem)
      
      err_fem[k, i] <- err_fem[k, i] + fem_kl
    }  # end for k in m_vec
...
final_filename <- paste0(
  "kl_tables/posterior_kl_rat_nngp_fem_range", range,
  "_sigmaE", sigma.e,
  "_nu", nu, ".RDS"
)
