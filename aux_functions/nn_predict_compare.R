...
for(kk in 1:n.rep) {
    for(i in 1:length(nu.vec)) {
        ...
        for(j in 1:length(m.vec)) { 
            ...
            t1 <- Sys.time()
            Qnn <- get.nnQ(loc = loc,kappa = kappa,nu = nu,sigma = sigma, n.nbr = mn)
            t2 <- Sys.time()
            ...
            t2 <- Sys.time()
            Qhat.rat <- Q + t(Qrat$A)%*%Qrat$A/sigma.e^2        
            mu.rat <- Qrat$A%*%solve(Qhat.rat, t(Qrat$A)%*%Y/sigma.e^2)
            t3 <- Sys.time()
...
plot(nu.vec,t.rat1[,1]+t.rat[,1],type="l", log= "y", ylim = c(min(c(min(t.rat1),min(t.nn1))),
                                                   max(c(max(t.rat1),max(t.nn1)))),
     main = "time total")
